...
class ContentParser:
    ...
    def parse_text(self, text):
        ...
        while i < len(lines):
            ...
            # 跳过标记块（当前仅用于目录 [TOC]）
            if skipping_block:
                if line.strip().upper() == f'[/{"TOC" if skipping_block=="TOC" else skipping_block}]':
                    skipping_block = None
                i += 1
                continue
            ...
            # 识别正文开始
            if line.startswith('[BODY]') or re.match(r'^第[一二三四五六七八九十\d]+章', line) or re.match(r'^[一二三四五六七八九十]+、', line):
            ...
            # 识别一级标题（章） 支持 “第X章 …” 和 “一、…” 两类格式
            match1 = re.match(r'^第([一二三四五六七八九十\d]+)章\s+(.+)$', line)
            match2 = re.match(r'^([一二三四五六七八九十]+)、\s*(.+)$', line)
            match3 = re.match(r'^(\d+)\s+(.+)$', line)

            if current_section == 'body' and (match1 or match2 or match3):
                ...
                if match1:
                    ...
                    chapter_num = self._chinese_to_arabic(match2.group(1))
                    chapter_title = match2.group(2).strip()
                else:
                    chapter_num = int(match3.group(1))
                    chapter_title = match3.group(2).strip()
            ...
            # 识别二级标题（节）：支持 “（一）”、“(一)” 或阿拉伯 1.1
            match_h2_cn = re.match(r'^（([一二三四五六七八九十]+)）\s*(.+)$', line)
            match_h2_cn_alt = re.match(r'^\(([一二三四五六七八九十]+)\)\s*(.+)$', line)
            match_h2_num = re.match(r'^(\d+\.\d+)\s+(.+)$', line)
            if current_chapter and (match_h2_cn or match_h2_cn_alt or match_h2_num):
                if current_paragraph:
                    para_text = ' '.join(current_paragraph)
                    current_chapter['content'].append({
                        'type': 'paragraph',
                ...
                section_counter += 1
                subsection_counter = 0
                section_cn = self._arabic_to_chinese(section_counter)

                if match_h2_num:
                    heading_text = match_h2_num.group(2).strip()
                elif match_h2_cn:
                    heading_text = match_h2_cn.group(2).strip()
                else:
                    heading_text = match_h2_cn_alt.group(2).strip()

                current_chapter['content'].append({
                    'type': 'heading2',
                    'number': f'第{section_cn}节',
                    'text': heading_text
                })
                i += 1
                continue

            # 识别三级标题：支持中文“一、”或阿拉伯“1.”/“1.1.1”
            match_h3_cn = re.match(r'^([一二三四五六七八九十]+)[、\.．]\s*(.+)$', line)
            match_h3_num = re.match(r'^(\d+)[\.．]\s*(.+)$', line)
            match_h3_full = re.match(r'^(\d+\.\d+\.\d+)\s+(.+)$', line)
            if current_chapter and (match_h3_cn or match_h3_num or match_h3_full):
                if current_paragraph:
                    para_text = ' '.join(current_paragraph)
                    current_chapter['content'].append({
                        'type': 'paragraph',
                ...
                sub_cn = self._arabic_to_chinese(subsection_counter)

                if match_h3_cn:
                    heading_text = match_h3_cn.group(2).strip()
                elif match_h3_num:
                    heading_text = match_h3_num.group(2).strip()
                else:
                    heading_text = match_h3_full.group(2).strip()
    ...
    def _arabic_to_chinese(self, number):
        ...
        tens_part = mapping[tens] + '十'
        ones_part = mapping[ones] if ones else ''
