...
- **特别说明**：Word 对自动生成的目录（TOC）条目通常不会写入显式的对齐属性，这类记录在 JSON 中会显示为“未提供”。请把它视为“缺少数据”而不是“实际居中/错误”，并且可以将这个目录的特别现象记为 0 权重。
...
- **特别说明（图表来源行）**：以“来源：”或 “Source:” 开头的段落属于图/表的来源说明，允许保持正文样式或首行缩进为空。请勿把它们当作正文缺陷或图表标题错误。
